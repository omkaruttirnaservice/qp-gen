...
function TestQuestionsView() {
    const { questionsList: testQuestionsList, testDetails: previewTestDetails } = useSelector(
        (state) => state.tests
    );
    console.log(previewTestDetails, '=testDetails');
    ...
    useEffect(() => {
        if (testQuestionsList.length == 0) {
            dispatch(getQuestionsListThunk(previewTestDetails.test_id, sendRequest, navigate));
        }
    }, [testQuestionsList]);
...
function QuestionSplitView({ testQuestionsList, renderTopicHeader, handleEditQuestion }) {
    return (
        <>
